...
<div>
  <p>The chart above shows the results of an automated audit to determine whether CALLSIGN (NODE_ID) can accept incoming connections.</p>
  <p>In order to effectively manage the network, the FreeSTAR admin team occasionally need to move connections from busy nodes to quieter ones. If a repeater or gateway does not accept incoming connections, it is nor possible to reconnect it to a quieter node without keeper intervention.</p>
